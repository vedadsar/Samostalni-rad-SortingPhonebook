...
public class CompareByFirstName implements Comparator {
	...
	public int compare(Person left, Person right){
		...
		char[] leftName = left.getFirstName().toCharArray();
		char[] rightName = right.getFirstName().toCharArray();
	...
	private float [] charToFloat(char [] array){
		...
		for(int i=0; i<array.length; i++)		//Turning char array to float array.
			floatArray[i] = (float)array[i];
		
		
		//Loop where we checks for special symbols(characters)
